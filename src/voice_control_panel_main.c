...
Eina_Bool _test_gpio(void *data)
{
	...
	if (resource_write_led(GPIO_SWITCH_A, cmd)) {
		LOGE("Cannot open FRONT");
	}

	if (resource_write_led(GPIO_SWITCH_B, cmd)) {
		LOGE("Cannot open BACK");
	}
...
#if 0
	if (resource_write_led(GPIO_FRONT, 0)) {
		LOGE("Cannot open FRONT");
	}

	if (resource_write_led(GPIO_BACK, 0)) {
		LOGE("Cannot open BACK");
	}

	if (resource_write_led(GPIO_SWITCH_A, 0)) {
		LOGE("Cannot open A");
	}

	if (resource_write_led(GPIO_SWITCH_B, 0)) {
		LOGE("Cannot open B");
	}
#endif
...
#if 0 // Test
	Ecore_Timer *timer = ecore_timer_add(10.0f, _test_gpio, NULL);
